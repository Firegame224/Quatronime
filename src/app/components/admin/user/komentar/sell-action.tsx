...
import { KomenColumn } from "./komentar";
import { Button } from "@/components/ui/button";
import { Copy, MoreHorizontalIcon, Trash2Icon } from "lucide-react";
...
const SellAction: React.FC<ActionProps> = ({ data }) => {
  ...
  const onDelete = async () => {
    try {
      setIsLoading(true);
      setIsOpen(true);
      const response = await fetch(`/api/users/${data.userId}/komentar/${data.id}`, {
        method: "DELETE",
        headers: {
      ...
      if (response.ok) {
        toast.success("Data komentar user Berhasil dihapus");
        router.refresh();
